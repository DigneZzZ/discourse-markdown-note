...
### 🔧 ИСПРАВЛЕНО
- **Настройки скрытия иконок и заголовков теперь работают корректно**
  - Переработана функция `parseBooleanSetting` для правильной обработки настроек Discourse
  - Добавлена поддержка всех форматов булевых значений (string, boolean, number)
  - Исправлены значения по умолчанию

### 🎨 УЛУЧШЕНО
- **Усилены CSS-правила скрытия элементов**
  - Добавлены ультра-специфичные селекторы с высоким приоритетом
  - Множественные методы скрытия: `display: none`, `visibility: hidden`, `opacity: 0`
  - Улучшено покрытие всех областей контента

### 🐛 ОТЛАДКА
- **Расширено логирование и диагностика**
  - Детальная инспекция состояния элементов (display, visibility, opacity)
  - Отложенные повторные применения настроек (5с и 10с после инициализации)
  - Улучшенные сообщения для отладки состояния настроек

### 🧪 ТЕСТИРОВАНИЕ
- Создан улучшенный тестовый файл `test-settings-v2.html`
- Добавлен подробный план тестирования в `TESTING-0.8.5.md`
- Функция принудительного обновления стилей для медленных подключений

## [0.8.4] - 2025-07-08 - ИСПРАВЛЕНИЕ ОШИБКИ ИНИЦИАЛИЗАТОРА

### 🚨 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ

- **ИСПРАВЛЕНА ОШИБКА "must export an initializer"**: Создан корректный файл `markdown-note-admin.js.es6`
- **УСИЛЕНА ЗАЩИТА ОТ АДМИНКИ**: Добавлены дополнительные проверки во все обработчики событий
- **ПОЛНАЯ ИЗОЛЯЦИЯ**: Все функции плагина теперь проверяют админский контекст перед выполнением

### 🔧 Технические изменения

- **Создан минимальный admin initializer** для предотвращения ошибок
- **Все обработчики событий обновлены** с проверками админского роута
- **MutationObserver изолирован** от админки
- **scheduleStyleUpdate защищен** дополнительными проверками

### 🎯 Результат

- Исчезла ошибка `discourse/plugins/discourse-markdown-note/initializers/markdown-note-admin must export an initializer`
- Полная изоляция от админского интерфейса
- Стабильная работа в контентных областях

## [0.8.3] - 2025-07-08 - УСИЛЕННАЯ ЗАЩИТА ОТ КОНФЛИКТОВ С АДМИНКОЙ

### 🚨 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ

- **ПОЛНОСТЬЮ ПЕРЕРАБОТАНА ЗАЩИТА ОТ КОНФЛИКТОВ**: Плагин теперь полностью изолирован от админки Discourse
- **РАСШИРЕННАЯ ПРОВЕРКА АДМИНСКИХ РОУТОВ**: Добавлены множественные проверки для исключения инициализации в админке
- **СПЕЦИФИЧНЫЕ CSS СЕЛЕКТОРЫ**: Все стили ограничены только областями контента

### 🔧 Технические изменения

- **JavaScript улучшения**:
  - Проверка роутов, классов `admin-interface` и админских контроллеров
  - Дополнительные проверки в `applyNoteStyles()` перед применением классов
  - Пропуск применения CSS классов в админских интерфейсах

- **CSS изоляция**:
  - Все селекторы обернуты в `body:not(.admin-interface):not([class*="admin"])`
  - Стили ограничены областями: `#main-outlet`, `.topic-body`, `.post-stream`, `.cooked`
  - Полное исключение возможности влияния на элементы админки

### 📋 Файлы для диагностики

- Добавлен `ADMIN-BUTTON-DEBUG.md` с инструкциями по диагностике
- Подробные логи для отладки конфликтов

## [0.8.2] - 2025-07-08 - КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ

### 🚨 КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ

- **ИСПРАВЛЕН КОНФЛИКТ С АДМИНКОЙ DISCOURSE**: CSS правила плагина затрагивали элементы админки, включая кнопку "Обновить Discourse" в `/admin/update`
- **ИСПРАВЛЕНА РАБОТА НАСТРОЕК**: Настройки `discourse_markdown_note_show_titles` и `discourse_markdown_note_show_icons` не работали корректно для новых типов заметок

### 🔧 Технические изменения

- **CSS селекторы сделаны более специфичными** для предотвращения конфликтов с админкой
- **Добавлена проверка роутов** для исключения инициализации плагина на админских страницах
- **Улучшена обработка булевых значений** в настройках (поддержка boolean и string)
- **Расширено логирование** для диагностики проблем с настройками
- **Добавлены ограничения области действия CSS** только для областей контента

### 🎯 Области изменений

- Селекторы теперь ограничены `#main-outlet`, `.topic-body`, `.post-stream`
- JavaScript исключает админские страницы (`/admin/*`)
- Улучшена совместимость с экосистемой Discourse

## [0.8.1] - 2025-07-08

### 🐛 Исправление отображения иконок и заголовков

- **ИСПРАВЛЕНА КРИТИЧЕСКАЯ ПРОБЛЕМА** с настройками `show_titles` и `show_icons`
- **Добавлены глобальные CSS селекторы** для корректного скрытия иконок и заголовков
- **Улучшена логика определения настроек** - теперь используется строгое сравнение `=== true`
- **Добавлено детальное логирование** для диагностики проблем с настройками
- **Исправлено скрытие псевдоэлементов** `::before` содержащих эмодзи иконки

### 🔧 Технические изменения

- Добавлены глобальные селекторы `body.hide-note-icons` и `body.hide-note-titles` с `!important`
- Обновлена логика в `note-theme-settings.js.es6` и `notifications.js.es6`
- Улучшена обработка отступов при скрытии иконок

## [0.8.0] - 2025-07-08

### ✨ Новые типы заметок

- **ДОБАВЛЕНЫ 5 НОВЫХ ТИПОВ ЗАМЕТОК:**
  - `[tip]` - советы и подсказки (💡)
...
- **Автоматические цветовые схемы** для всех новых типов
- **Уникальные эмодзи иконки** для каждого типа
- **Градиентные фоны** с поддержкой прозрачности
- **Адаптивные цвета** для светлой и темной тем

### 🔄 Расширенная совместимость

- **Legacy поддержка** новых типов через `[note type=""]`
...
- Добавлены настройки для всех новых типов в admin панель
- Обновлены переводы на русский и английский языки
- Расширены CSS переменные для кастомизации
- Улучшена обработка ошибок в theme settings

### 📋 Полный список поддерживаемых типов

Теперь плагин поддерживает **11 типов заметок:**
1. `note` - обычные заметки
2. `info` - информация
3. `warn` - предупреждения
4. `negative` - критические уведомления
5. `positive` - успешные операции
6. `caution` - осторожность
7. `tip` - советы (НОВЫЙ)
8. `todo` - задачи (НОВЫЙ)
9. `bug` - ошибки (НОВЫЙ)
10. `feature` - новые функции (НОВЫЙ)
11. `security` - безопасность (НОВЫЙ)
