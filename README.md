...
### Старые типы (автоматически конвертируются):
```
[negative]Ошибка (конвертируется в error)[/negative]
[positive]Успех (конвертируется в success)[/positive]
[caution]Важно (конвертируется в important)[/caution]
[attention]Важно (конвертируется в important)[/attention]
[tip]Информация (конвертируется в info)[/tip]
```
