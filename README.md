...
- ✅ **Кнопка в редакторе** с выпадающим меню для быстрой вставки заметок
- ✅ **Интерактивный выбор цвета** в админ-панели (color picker)
- ✅ Гибкие настройки через стандартную панель настроек плагина
- ✅ Локализация на русском и английском языках
- ✅ Адаптивный дизайн для мобильных устройств
- ✅ Accessibility: поддержка `role="note"` и `aria-hidden`
...
Для настройки плагина перейдите в:
**Админ → Настройки → Плагины → discourse_markdown_note**
...
| `theme_mode` | Режим темы: auto, light, dark |
| `show_titles` | Показывать заголовки заметок |
| `show_icons` | Показывать иконки-эмодзи |

### Настройка цветов:

Для каждого из 8 типов заметок можно настроить:
- 🎨 **Цвет границы** — интерактивный color picker
- 🎨 **Цвет текста (светлая тема)** — интерактивный color picker
- 🎨 **Цвет текста (тёмная тема)** — интерактивный color picker
- 📝 **Фон (светлая/тёмная тема)** — CSS градиенты

## Кнопка в редакторе

При создании или редактировании сообщения в панели инструментов появляется кнопка 📋 с выпадающим меню всех типов заметок:

1. Нажмите на кнопку заметки в редакторе
2. Выберите нужный тип из выпадающего списка
3. BBCode тег будет вставлен автоматически
...
assets/
  javascripts/
    ...
    initializers/
      ...
      note-editor-button.js.es6    # Кнопка редактора с выпадающим меню
  stylesheets/
...
### v1.1.3 (Текущая) - Исправление редактора
- ✅ Исправлена вставка заметок в редакторе - теперь выделенный текст корректно оборачивается BBCode тегами
- ✅ Использован метод `applySurround()` вместо `addText()` для правильной обработки выделенного текста
- 
### v1.1.2  - Исправления кнопки редактора
- ✅ Исправлена иконка кнопки в редакторе (теперь отображается корректно)
- ✅ Улучшен поиск кнопки для позиционирования dropdown
- ✅ Добавлено фиксированное позиционирование выпадающего меню

### v1.1.1 - Исправления dropdown
- ✅ Исправлено позиционирование выпадающего меню
- ✅ Добавлен fallback при отсутствии кнопки

### v1.1.0 - Редактор и UX
- ✅ **Кнопка в редакторе** с выпадающим меню для быстрой вставки заметок
- ✅ **Интерактивный color picker** в админ-панели для выбора цветов
- ✅ Рефакторинг JS: устранение дублирования кода, вынос в функции
- ✅ Добавлена поддержка i18n для заголовков заметок
- ✅ Опциональное логирование через DEBUG флаг
- ✅ Accessibility: добавлены `role="note"` и `aria-hidden` для иконок
- ✅ Оптимизация отступов блоков (~37% компактнее)
- ✅ Исправлены сломанные эмодзи в CSS (🔥, 💡)
- ✅ Обновлён prettier до версии 3.4.2

### v1.0.0 - Оптимизация типов заметок
